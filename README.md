...
I could find some templates (available at "**Useful links**" sections at the bottom), but I would like a minimalist approach instead of showing all follow-ups and tasks in a single mail. I found [hmarthe/TCAT-M-osTicket-Email-Templates](https://github.com/hmarthe/TCAT-M-osTicket-Email-Templates) and really liked it, so I decided to adapt this template from osTicket to GLPI.

This template was successfully tested on:
...
*Novo acompanhamento (New follow-up)/Atualização de um acompanhamento (Update of a follow-up).*

![Novo acompanhamento (New follow-up)](/screenshots/new-or-update-task.png?raw=true "Nova tarefa (New task)/Atualização de uma tarefa (Update of a task)")
...
Another function that depends on translated strings is ``ticket.isprivate``, e.g., to detect that a follow-up or task has been marked as private, it uses the function ``##IFticket.isprivate=Não##``. "**Não**" is the translation to "No" in "Português do Brasil".
...
**Note**: once the HTML is pasted into GLPI and saved, the source code view in GLPI may modify the HTML. Elements like comment tags () and other formatting may be removed or altered. Because of this, you shouldn’t use the version displayed in GLPI as a basis for future edits—always go back to your original file.
...
1. Select your language in **Language** dropdown, e.g., "Português do Brasil".
...
1. [Notificações no GLPI em HTML e CSS que funcionam no Gmail](http://www.thiagopassamani.com.br/tags/glpi-notification-template-mail). Thiago Passamani.
